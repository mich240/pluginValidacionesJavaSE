...
//	 6);
	...
	public static void addComportamiento(Object... Campo_Limite_TipoValidacion) {
		int a = 0, b = 1, c = 2;
		/**
		 * recorro cada campo el primero es un campoTexto el segundo es el
		 * limite y el tercero es lo q voy a validar
		 */
		for (int i = 0; i < Campo_Limite_TipoValidacion.length; i++) {
			/**
			 * al conteo de 3 entro al if es decir entro asi al 3 luego al 6
			 * luego al 9
			 */
			if (i % 3 == 0) {
				/**
				 * es decir que hago brincos de 3 en tres, y es asi xq ahora uso
				 * el metodo cargaValida para cada elemente si los parametros
				 * son correctos serian como dije arriba: un texto - limte -
				 * validacion
				 */
				cargaValida(Campo_Limite_TipoValidacion[a]);// texto
				/**
				 * basicamente la carga valida me identifica que atributo le voy
				 * a dar a cada uno de esos campos sera 3 los q enviare y los
				 * guardare en mis variables globales y se les asignara el
				 * atributo correspondiente todo esto suce en un solo cliclo de
				 * for es decir q por cada ciclo yo cambio esos campos ya q son
				 * globales.
				 */
				cargaValida(Campo_Limite_TipoValidacion[b]);// limite
				cargaValida(Campo_Limite_TipoValidacion[c]);// validacion
				/**
				 * como dije antes una ves q defino las 3 variables globales con
				 * estas locales le doy sus respectivos atributos
				 */
				atribuirValores();
				/**
				 * es mi forma de aumentar los valores de las varibles
				 * incrementales el caso es q ellas deben sufir de 3 en tres asi
				 * q lo q hice fue igualar a al c puesto q c es el ultimo osea 3
				 * (en primera instancia) le sumo 1 y queda en 4 y en base a eso
				 * aumento b y c quedando para los proximos valores 4,5,6 como 6
				 * es divisible entre 3 entra en el if en esa vuelta, y se toman
				 * los siguientes 3 campos text-limite-validacion.
				 */
				a = c + 1;
				b = a + 1;
				c = b + 1;
	...
	private static void cargaValida(Object valor) {
		/**
		 * si es un componente de texto lo almaceno en componente y si es un
		 * numero hay 2 posibles opciones: si el valor es mayor a 1 es el tipo
		 * de cantidad q deseo limitar en el campo. de resto es el tipo de
		 * validacion puesto q para el tipo de validacion use valores negativos
		 * para q no interfiran por si una persona quiere colocar como cantidad
		 * limite un 1 o 2 xq es posible.
		 */
	...
	private static void atribuirValores() {
		/**
		 * estas son las variables q se especifican en la cargavalida separando
		 * los 3 elementos de lso campos
		 */
		...
		/**
		 * tomamos al componente y a�adimos el KeyReleased. el es metodo q se
		 * ejecuta al quitar el dedo del boton presionado
		 */
			...
			public void keyReleased(KeyEvent event) {
				...
				if (component instanceof JTextComponent) {/// nos aseguramos de
					...
					if (((JTextComponent) component).getText().length() > cant) {
						/**
						 * si el tama�o de los caracteres del campo es mayor a
						 * la cantidad limite q estableci, guardo en 1 variable
						 * toda la cadena desde 0 hasta la cantidad q estableci
						 * digamos q fue 8. eso tomaria la cadena de ese
						 * componente de 0 a 8 y se lo seteo al campo eso sucede
						 * al despresionar la tecla del caracter q se este
						 * digitando en ese momento lo q hace q sea dinamico y
						 * inmediatamente q se tipee se seteara de forma
						 * correcta el valor de 0 a 8 tambien te salva de la
						 * posibilidad de q la persona se quede presionando la
						 * tecla hasta llenar el campo al soltarlo se enviara la
						 * cadena correcta 0 a 8 ejemplo.
						 **/
						...
						((JTextComponent) component).getToolkit()
								.beep();/**
										 * hago el sonido de beep para q el
										 * usuario se entere del error
										 */
					}
					/**
					 * ahora viene la valicacion del tipo. use 2 posibles
					 * valores 0: es para validar q solo se permitan numeros.
					 * -1: es para validar q solo se permitan letras. -2: el -2
					 * en realidad esta demas puesto q solo valido si es 0 o -1
					 * si no son esos valores es xq automaticamente quiero q la
					 * validacion sea alfanumerica es decir q pasar cualquier
					 * valor diferente de -1 y 0 hara q sea alfanumerico en mi
					 * variable contante definida en util uso NUMBER_AND_LETTER
					 * con -2
					 */
					if (tip == 0) {
						String texto = "";
						for (int i = 0; i < ((JTextComponent) component).getText().length(); i++) {
							/**
							 * recorro cada caracter de la cadena de texto de mi
							 * campo, a medida q recorro la cadena obtengo el
							 * caracter correspondiente y verifico si es
							 * numerico con el metodo isNumeric
							 */
							if (isNumeric(new Character(((JTextComponent) component).getText().charAt(i))))
								texto += ((JTextComponent) component).getText().charAt(i);
							/**
							 * si el caracter es numerico lo agrego a mi
							 * variable texto de esa manera a medida q verifico
							 * caracteres cuando encuentre uno q no es numerico
							 * no lo guardara en este texto. al final seteo ese
							 * texto al campo limpio de caracteres no numericos.
							 * lo hago asi xq si el usaurio presiona muchas
							 * teclas a la ves y se pasa una letra por alli,
							 * haciendolo de esta manera al soltar el todas la
							 * teclas se lanza este metodo y limpiaria toda la
							 * cadena.
							 */
							else
								((JTextComponent) component).getToolkit()
										.beep();/**
												 * si no es numerico uso el
												 * sonido del beep para avisarle
												 * al usuario
												 */
						}
					...
					if (tip == -1) {
						...
						for (int i = 0; i < ((JTextComponent) component).getText().length(); i++) {
							/**
							 * basicamente hace lo mismo q el del numerico de
							 * esta forma admito todo los caracteres excepto
							 * numeros asi q se pueden tipear letras espacios
							 * simbolos
							 */
							if (!isNumeric(new Character(((JTextComponent) component).getText().charAt(i))))// tomo
																											// los
																											// caracteres
																											// no
																											// numericos
	...
	private static void addEventError(final JTextComponent field) {
		if (field.getBorder() != borderErros) {
			...
			System.out.println("sete border a texto");
			field.addKeyListener(new KeyAdapter() {
				@Override
				public void keyPressed(KeyEvent e) {
					if (field.getBorder() == borderErros) {
						field.setBorder(borderDefecto);
						System.out.println("sete border defectoText"+field);
					}
				}
			});
		}
	...
	private static void addEventError(final JComboBox<?> field) {
		...
		if (field.getBorder() != borderErros) {
			final Border borderDefecto = field.getBorder();
			field.setBorder(borderErros);
			System.out.println("sete border a combo");
...
//			});
			field.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (field.getBorder() == borderErros) {
						field.setBorder(borderDefecto);
						System.out.println("sete border defectoCombo"+field);
					}
				}
			});
	...
	public static boolean field(Object... fields) {
		/**
		 * una variable para cada validacion por ahora solo 3 campos:vacios,
		 * campos con el primero caracter en blanco y comoBox con el primer
		 * indice seleccionado.
		 */
		boolean vacio = false, blanco = false, primeroIndex = false;
		int numeroErrores = 0;/**
								 * contador de errores, este contador va a
		...
		for (Object each : fields) {
			...
			if (each instanceof JTextComponent) {
				...
				if (((JTextComponent) each).getText().isEmpty()) {
					/**
					 * si mi campo de texto esta vacio cambio vacio a true y
					 * aumento un valor a numeroErrores pero si otro campo me
					 * llega a dar este error ya no incremento numeroErrores
					 */
					if (!vacio) {
						vacio = true;
						numeroErrores++;
					}
					System.out.println("cai en vacio");
					addEventError(((JTextComponent) each));
					
				} else if (String.valueOf(((JTextComponent) each).getText().charAt(0)).equals(" ")) {
					/**
					 * si el primer caracter es un campo en blanco cambio blanco
					 * a true y aumento un valor en numeroErrores. si caigo aqui
					 * existe la posibilidad de tener 2 errores de tipo distinto
					 * y si otro campo da este error no debe aumentar a
					 * numeroErrores
					 */
					if (!blanco) {
						blanco = true;
					...
					System.out.println("cai en blanco");
					addEventError(((JTextComponent) each));
			...
			if (each instanceof JComboBox) {
				...
				if (((JComboBox<?>) each).getSelectedIndex() == 0) {
					/**
					 * yo usare el primer index de un combo para validar los
					 * jcombox. si tengo seleccionado el primero index de un
					 * combo me da error de "seleccione una opcion" cambio
					 * primeroIndex a true y aumento un valor a numeroErrores,
					 * asi serian 3 errores dinstintos y si otro Objeto da este
					 * error no aumento nada a numeroErrores.
					 */
					if (!primeroIndex) {
						primeroIndex = true;
						numeroErrores++;
					}
					System.out.println("cai en combo");
		...
		/**
		 * despues de haber recorrido todos los Objetos verifico si hay errores.
		 * si son mas de 1 errores entonces tengo multiple fallas es decir
		 * campos vacios, campos en blanco al inicio y comboBox no
		 * seleccionados.
		 */
		if (numeroErrores > 1) {
		...
		if (numeroErrores == 1) {
			/**
			 * si solo tube un error significa q puedo filtrarlo y mostrar un
			 * msj mas personalizado a los campos q tubieron ese error.
			 */
			if (vacio) {
